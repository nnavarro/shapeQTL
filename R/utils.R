...
#' 
#' A function to merge new phenotypes (eg tangent coordinates) in a cross read by read.cross.
#' 
#' Function takes a cross object and a dataframe with new phenotypes, matches cross$pheno$id to 
...
#' @param phen2update An optional vector with names of the subset of new phenotypes to keep
#' @param id.geno An optional vector with the matching id in the genotypes
#' @param na.rm An optional logical if individuals with missing phenotypes should be removed from the cross
#' @export
...
#' fake.bc <- update.cross(fake.bc, tgCoords, phen2keep=c("Sex","age"), phen2update=grep("PC",colnames(tgCoords)))

update.cross <- function(cross,new.pheno=NULL,phen2keep=NULL,phen2update=NULL,id.geno=NULL,na.rm=FALSE,...){
	if(!any(class(cross)=="cross")) stop("cross must be a R/qtl cross object")
	...
	if (any(phen2keep%in%id)) phen2keep <- phen2keep[-c(phen2keep%in%id)]
	idx <- match(phen2keep,colnames(cross$pheno),nomatch=0)
	idx <- idx[idx>0]
	if (!length(idx)) cross$pheno <- data.frame(ID=ID)
	else{  
	...
	if (!is.null(new.pheno)){
		...
		if (is.null(id.geno)) 
			id.geno <- match(cross$pheno[,colnames(cross$pheno)%in%id], new.pheno[,colnames(new.pheno)%in%id])
		new.pheno <- new.pheno[id.geno,]
	...
	if (!(!L))
		for (i in 1:L) {
			...
			clnms <- c(clnms,paste(cov.names,1:ifelse(is.null(ncol(args[[i]])),1,ncol(args[[i]])),sep="."))
			}
...
#' geno <- get.geno(fake.bc, Q)

get.geno <- function(cross,Q,add.only=FALSE){
	...
	for (q in 1:n.qtl){
		...
		if (class(cross)[1]=="bc") pr <- pr[,,-dim(pr)[3],drop=TRUE]
		else {
			if(class(cross)[1]=="f2" & add.only) pr <- pr[,,2]+2*pr[,,3]  
			else pr <- pr[,,-dim(pr)[3],drop=TRUE]	
		}
		if (is.na(dim(pr)[3])) geno <- cbind(geno,pr[,q1])
		else geno <- cbind(geno,pr[,q1,1:(n.gen[q]-1)])
	}
	gen <- NULL
	if(class(cross)[1]=="f2" & add.only) n.gen <- n.gen - 1
	for (q in 1:n.qtl) gen <- c(gen,paste(paste("q",q,sep=""),1:(n.gen[q]-1),sep="."))
	colnames(geno) <- gen	
